...
from q23.dashboard.components.styles import (
    ...
    get_strategy_color_manager,
    get_strategy_color,
)

# =============================================================================
# SOLID DESIGN: Interfaces and Abstract Base Classes
# =============================================================================
...
class IStrategyManager(Protocol):
    """Interface for strategy management operations."""
    ...
    def get_enabled_strategies(self) -> Dict[str, bool]:
        """Get enabled status for all strategies."""
        ...

    def set_strategy_enabled(self, strategy_id: str, enabled: bool) -> None:
        """Set enabled status for a strategy."""
        ...

    def save_enabled_strategies(self, enabled: Dict[str, bool]) -> bool:
        """Save enabled strategies configuration."""
        ...


class IStrategyMetricsProvider(Protocol):
    """Interface for strategy metrics and data access."""

    def get_strategy_configs(self) -> dict:
        """Get configuration for all strategies."""
        ...

    def get_strategy_metrics(self, strategy_id: str, tag: str) -> Dict[str, Any]:
        """Get performance metrics for a strategy."""
        ...

    def get_strategy_tags(self, strategy_id: str) -> List[str]:
        """Get available tags for a strategy."""
        ...


class IWarehouseSection(ABC):
    """Abstract base class for warehouse sections."""

    def __init__(self, strategy_manager: IStrategyManager, metrics_provider: IStrategyMetricsProvider):
        self.strategy_manager = strategy_manager
        self.metrics_provider = metrics_provider

    @abstractmethod
    def render(self, strategies: List[str]) -> None:
        """Render this section of the warehouse."""
        ...
...
class StrategyManager(IStrategyManager):
    ...
    def get_available_strategies(self) -> List[str]:
        """Get all available strategies including benchmarks."""
        return discover_available_strategies(include_benchmarks=True)

    def get_enabled_strategies(self) -> Dict[str, bool]:
        """Get enabled status with auto-enable for benchmarks."""
        enabled = _load_enabled_strategies()
        return _auto_enable_benchmarks_with_runs(enabled)

    def set_strategy_enabled(self, strategy_id: str, enabled: bool) -> None:
        """Set enabled status for a strategy."""
        current = self.get_enabled_strategies()
        current[strategy_id] = enabled
        self.save_enabled_strategies(current)

    def save_enabled_strategies(self, enabled: Dict[str, bool]) -> bool:
        """Save enabled strategies configuration."""
        return _save_enabled_strategies(enabled)


class StrategyMetricsProvider(IStrategyMetricsProvider):
    """Concrete implementation of metrics provider."""
    ...
    def get_strategy_metrics(self, strategy_id: str, tag: str) -> Dict[str, Any]:
        """Get performance metrics for a strategy."""
        return _load_run_metrics(strategy_id, tag)
...
class StrategyOverviewSection(IWarehouseSection):
    """Handles the strategy overview table section."""

    def render(self, strategies: List[str]) -> None:
        """Render strategy overview table with enable/disable controls."""
        st.subheader("📊 Registered Strategies")

        enabled_config = self.strategy_manager.get_enabled_strategies()
        configs = self.metrics_provider.get_strategy_configs()
        ...
        for sid in strategies:
            ...
            tags = self.metrics_provider.get_strategy_tags(sid)
            latest_tag = tags[0] if tags else None

            # Load metrics from latest run
            metrics = self.metrics_provider.get_strategy_metrics(sid, latest_tag) if latest_tag else {}
        ...
        # Convert metrics columns to strings
        ...
        with col_save:
            if st.button("💾 Save Enable/Disable Settings", type="primary"):
                new_enabled = {}
                for _, row in edited_df.iterrows():
                    new_enabled[row["ID"]] = bool(row["Enabled"])

                if self.strategy_manager.save_enabled_strategies(new_enabled):
        ...
        with col_info:
            enabled_count = sum(1 for _, row in edited_df.iterrows() if row["Enabled"])
            st.caption(f"{enabled_count} strategies enabled for startup runs")


class StrategyComparisonSection(IWarehouseSection):
    """Handles strategy comparison functionality."""
    ...
    def render(self, strategies: List[str]) -> None:
        ...
        color_map = build_strategy_color_map(strategies)
        ...
        if compare_strategies:
            self._render_comparison_charts(compare_strategies)
        else:
            st.caption("Select 2-4 strategies to compare their performance.")

    def _render_comparison_charts(self, strategy_ids: List[str]) -> None:
        """Render comparison charts for selected strategies."""
        # Implementation moved from original _render_comparison_charts
        perf_data = {}
        ...
        for sid in strategy_ids:
            tags = self.metrics_provider.get_strategy_tags(sid)
            if tags:
                metrics = self.metrics_provider.get_strategy_metrics(sid, tags[0])
...
class ConfigDiffSection(IWarehouseSection):
    """Handles configuration difference viewing."""

    def render(self, strategies: List[str]) -> None:
        """Render configuration difference interface."""
        ...
        if left_strategy and right_strategy:
            self._render_config_diff(left_strategy, right_strategy)

    def _render_config_diff(self, left_id: str, right_id: str) -> None:
...
class RunHistorySection(IWarehouseSection):
    """Handles run history viewing."""

    def render(self, strategies: List[str]) -> None:
        """Render run history interface."""
        ...
        if selected_for_history:
            self._render_run_history(selected_for_history)

    def _render_run_history(self, strategy_id: str) -> None:
        """Render run history for a strategy."""
        tags = self.metrics_provider.get_strategy_tags(strategy_id)
        ...
        for tag in tags[:20]:  # Limit to most recent 20
            ...
            metrics = self.metrics_provider.get_strategy_metrics(strategy_id, tag)
...
class ColorManagementSection(IWarehouseSection):
    """Handles strategy color management."""

    def render(self, strategies: List[str]) -> None:
        """Render color management interface."""
        st.subheader("🎨 Strategy Colors")

        color_manager = get_strategy_color_manager()

        with st.expander("Customize Strategy Colors", expanded=False):
            col_color1, col_color2 = st.columns([2, 1])

            with col_color1:
                st.markdown("**Select Strategy to Customize:**")
                color_strategy = st.selectbox(
                    "Strategy",
                    options=strategies,
                    format_func=get_strategy_display_name,
                    key="color_strategy_select",
                    label_visibility="collapsed"
                )

            with col_color2:
                current_color = get_strategy_color(color_strategy)
                st.markdown(f"**Current Color:** {current_color}")
                st.markdown(
                    f'<div style="width: 50px; height: 20px; background-color: {current_color}; border: 1px solid #ccc; border-radius: 3px;"></div>',
                    unsafe_allow_html=True
                )

            # Color palette selection
            st.markdown("**Choose New Color:**")
            color_options = color_manager.get_color_palette_options()

            # Create a grid of color options
            cols = st.columns(8)
            selected_color = None

            for i, (color_name, hex_color) in enumerate(color_options.items()):
                col_idx = i % 8
                with cols[col_idx]:
                    if st.button(
                        color_name,
                        key=f"color_{color_name}_{i}",
                        help=f"Select {color_name} ({hex_color})",
            ...
            # Custom color input
            st.markdown("**Or Enter Custom Hex Color:**")
            ...
            # Apply color change
            if selected_color:
                color_manager.set_custom_color(color_strategy, selected_color)
                st.success(f"✅ Color updated for {get_strategy_display_name(color_strategy)}!")
                st.rerun()
            ...
            with col_reset1:
                if st.button("🔄 Reset Strategy Color", key="reset_single_color"):
                    color_manager.remove_custom_color(color_strategy)
                    st.success(f"✅ Reset color for {get_strategy_display_name(color_strategy)} to default!")
                    st.rerun()

            with col_reset2:
                if st.button("🔄 Reset All Colors", key="reset_all_colors", type="secondary"):
                    color_manager.reset_all_custom_colors()
                    st.success("✅ All strategy colors reset to defaults!")
                    st.rerun()

            # Preview current colors
            st.markdown("**Current Color Assignments:**")
            color_preview_data = []
            for sid in strategies:
                color = get_strategy_color(sid)
                color_preview_data.append({
                    "Strategy": get_strategy_display_name(sid),
                    "Color": color,
                    "Preview": f"⬜ ({color})"
                })

            color_df = pd.DataFrame(color_preview_data)
            st.dataframe(
                color_df,
                column_config={
                    "Preview": st.column_config.Column(
                        "Color Preview",
                        help="Visual color preview"
                    )
                },
                hide_index=True,
                use_container_width=True
...
class QuickActionsSection(IWarehouseSection):
    """Handles quick actions interface."""
    ...
    def render(self, strategies: List[str]) -> None:
        ...
        with col_a2:
            if st.button("✅ Enable All", key="warehouse_enable_all"):
                ...
                self.strategy_manager.save_enabled_strategies(all_enabled)
                st.rerun()

        with col_a3:
            if st.button("❌ Disable All", key="warehouse_disable_all"):
                all_disabled = {sid: False for sid in strategies}
                self.strategy_manager.save_enabled_strategies(all_disabled)
                st.rerun()
...
def render_strategy_warehouse() -> None:
    """Render the Strategy Warehouse page using SOLID architecture."""
    ...
    metrics_provider = StrategyMetricsProvider()
    ...
    # Create and render sections following Composition over Inheritance
    sections = [
        StrategyOverviewSection(strategy_manager, metrics_provider),
        StrategyComparisonSection(strategy_manager, metrics_provider),
        ConfigDiffSection(strategy_manager, metrics_provider),
        RunHistorySection(strategy_manager, metrics_provider),
        ColorManagementSection(strategy_manager, metrics_provider),
        QuickActionsSection(strategy_manager, metrics_provider),
    ]

    # Render each section with proper separation of concerns
    for section in sections:
        section.render(strategies)
        if section.__class__ != QuickActionsSection:  # Don't add divider after last section
            st.divider()
