#!/usr/bin/env python
"""Initialize cryptocurrency database schema.

This script creates the database schema (tables and indexes) for cryptocurrency market data storage.
...
def main():
    parser = argparse.ArgumentParser(
        description="Initialize cryptocurrency database schema"
    )
    parser.add_argument(
        "--drop-existing",
        action="store_true",
    ...
    try:
        ...
        if crypto_schema_exists(client):
            if args.drop_existing:
                logger.warning("Dropping existing schema...")
                if not drop_crypto_schema(client):
        ...
        logger.info("Creating cryptocurrency database schema...")
        if create_crypto_schema(client):
            logger.info("Crypto schema created successfully!")
            logger.info("Tables created: crypto_universe, crypto_ohlcv, crypto_ingestion_log")
        else:
            logger.error("Failed to create crypto schema")
            return 1
        
        # Create blockchain schema (if not exists)
        if not blockchain_schema_exists(client):
            logger.info("Creating blockchain metrics database schema...")
            if create_blockchain_schema(client):
                logger.info("Blockchain schema created successfully!")
                logger.info("Tables created: crypto_blockchain_metrics, crypto_blockchain_data, crypto_blockchain_ingestion_log")
            else:
                logger.error("Failed to create blockchain schema")
                return 1
        else:
            logger.info("Blockchain schema already exists")
